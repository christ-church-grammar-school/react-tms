...
def grade_file_url(url, test_name):
    ...
    filepath = os.path.join(os.path.join(
        os.path.dirname(os.path.realpath(__file__)), 'uploads/'),
                            filename)
    return run_tests(filepath, test_name)

def mark_student(upload, class_name):
    ...
    for test in grading_stream:
        ...
        if not test_dict['graded']:
            result = grade_file_url(student_upload['refs'][0], 'aplusb')
...
def mark_class(class_name):
    ...
    for upload in class_stream:
        mark_student(upload, class_name)
